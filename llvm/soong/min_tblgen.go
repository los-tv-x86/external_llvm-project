...
func init() {
	android.RegisterModuleType("llvm22_min_tblgen", llvm22MinTblgenFactory)
}

var (
	min_tblgen_pctx = android.NewPackageContext("android/soong/llvm22_min_tblgen")

	llvm22MinTblgen = min_tblgen_pctx.HostBinToolVariable("llvm22MinTblgen", "llvm22-min-tblgen")

	min_tblgenRule22 = min_tblgen_pctx.StaticRule("min_tblgenRule22", blueprint.RuleParams{
		Depfile:     "${out}.d",
		Deps:        blueprint.DepsGCC,
		Command:     "${llvm22MinTblgen} ${includes} ${generator} -d ${depfile} -o ${out} ${in}",
		CommandDeps: []string{"${llvm22MinTblgen}"},
		Description: "LLVM22 Min TableGen $in => $out",
		Restat:      true,
...
func llvm22MinTblgenFactory() android.Module {
	t := &min_tblgen{}
